...
4. Ottimizzazione performance
   ...
   - Implementato ramo sperimentale `QW3_METAL_DYNAMIC_ROUTER=1`: top-8/softmax router su GPU, buffer `routerIds/routerWeights` in sessione e MoE sparse con expert slot dinamici. Correttezza verificata (`--metal-session-decode-test -p ciao` passa), incluso mapping no-copy dei tensor expert quando attraversano le view GGUF.
   - Il ramo dynamic-router non e' default per ora: nella forma naive e' piu' lento (`--metal-run 2 -p ciao --ctx 128` con profilo circa 520-660 ms/token) del path CPU-assisted router. Va ottimizzato prima di sostituire il path stabile.
   - Aggiunta cache globale/lazy per il buffer costante `iq3_s kgrid`, evitando riallocazioni ripetute nei matvec expert.
   - Aggiunti kernel fusi `iq3_s` gate+up per sparse MoE default e dynamic-router. La correttezza resta invariata; il miglioramento e' piccolo perche' il collo di bottiglia dominante e' ancora il sync prima del router.
   ...
   - Misura default stabile dopo kgrid cache + gate/up fused: `--metal-run 8 -p ciao --ctx 128` circa `4072 ms`.
...
2. Ottimizzare il ramo `QW3_METAL_DYNAMIC_ROUTER=1` solo se si riesce a ridurre il costo GPU dei kernel expert-slot; al momento e' corretto ma piu' lento del path default.
