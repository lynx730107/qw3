...
4. Ottimizzazione performance
   ...
   - Cached attention GQA riscritta seguendo la direzione flash/vec di ds4/llama.cpp: un threadgroup per Q head calcola il dot Q*K una sola volta per token e accumula V con softmax online, invece di ricalcolare il dot per ogni dimensione dell'head. `--metal-session-gqa-cached2-test 66`, `--metal-gqa-attend4-test 66`, `--metal-gqa-branch4-test 66` e smoke passano. Misura `--metal-run 32 --metal-run-quiet`: circa `27.0 tok/s`; `--metal-run 64 --metal-run-quiet`: circa `26.8 tok/s`.
