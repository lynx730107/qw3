...
static const uint64_t QW3_GIB = 1024ull * 1024ull * 1024ull;
...
bool qw3_parse_gib_arg(const char *s, uint64_t *bytes) {
    ...
    if (errno != 0 || v == 0 || v > UINT64_MAX / QW3_GIB) return false;

    *bytes = (uint64_t)v * QW3_GIB;
    return true;
}
...
bool qw3_parse_streaming_cache_experts_arg(const char *s,
    ...
    if (len > 2 &&
        (s[len - 2] == 'g' || s[len - 2] == 'G') &&
        (s[len - 1] == 'b' || s[len - 1] == 'B')) {
        return qw3_parse_gib_arg(s, bytes);
    }
...
bool qw3_ssd_memory_lock_acquire(qw3_ssd_memory_lock *lock,
    ...
    if (ptr == MAP_FAILED) {
        fprintf(stderr,
                "qw3: --simulate-used-memory mmap %.2f GiB failed: %s\n",
    ...
    for (uint64_t off = 0; off < bytes; off += chunk_bytes) {
        ...
        if (mlock((void *)(p + off), (size_t)len) != 0) {
            fprintf(stderr,
                    "qw3: --simulate-used-memory mlock failed after %.2f/%.2f GiB: %s\n",
                    (double)locked / (double)QW3_GIB,
                    (double)bytes / (double)QW3_GIB,
                    strerror(errno));
    ...
    lock->bytes = bytes;
    fprintf(stderr,
            "qw3: simulated used memory: locked %.2f GiB before model load\n",
            (double)bytes / (double)QW3_GIB);
    return true;
}
