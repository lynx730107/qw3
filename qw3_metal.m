...
static NSString *qw3_metal_kernel_source(void) {
    return @"#include <metal_stdlib>\n"
            ...
            "        reg[0][j] = half(dl * float(g0[j]) * ((uint(s0) & (1u << j)) ? -1.0f : 1.0f));\n"
            "        reg[1][j] = half(dl * float(g1[j]) * ((uint(s0) & (1u << (j + 4u))) ? -1.0f : 1.0f));\n"
            "        reg[2][j] = half(dl * float(g2[j]) * ((uint(s1) & (1u << j)) ? -1.0f : 1.0f));\n"
            "        reg[3][j] = half(dl * float(g3[j]) * ((uint(s1) & (1u << (j + 4u))) ? -1.0f : 1.0f));\n"
            "    }\n"
            "    return reg;\n"
            "}\n"
            ...
            "            const short sx = short(2 * il0 + i / 8);\n"
            "            const short lx = short(i % 8);\n"
            "            *(sa + NK * (8 * sy_a + ly_a) + 8 * sx + lx) = k < args.n_in ? gate4[i / 4][i & 3] : half(0.0f);\n"
            "        }\n"
            "        int pid = pair_ids[map_base + uint(lr1)];\n"
            "        uint token = uint(pid) / args.n_active;\n"
            "        device const float *x = x1 + uint64_t(token) * uint64_t(args.n_embd) + uint64_t(loop_k);\n"
            ...
            "            const short sx = short(2 * il0 + i / 8);\n"
            "            const short lx = short(i % 8);\n"
            "            *(sa + NK * (8 * sy_a + ly_a) + 8 * sx + lx) = k < args.n_in ? up4[i / 4][i & 3] : half(0.0f);\n"
            "        }\n"
            "        threadgroup_barrier(mem_flags::mem_threadgroup);\n"
            "        sA = tA.slice(0, 0);\n"
            "        sB = tB.slice(0, 0);\n"
            ...
            "            const short lx = short(i % 8);\n"
            "            const short ly = short((tid / NL0) % 8);\n"
            "            *(sa + NK * (8 * sy + ly) + 8 * sx + lx) = k < args.n_ff ? w4[i / 4][i & 3] : half(0.0f);\n"
            "        }\n"
            "        int pid = pair_ids[map_base + uint(lr1)];\n"
            "        device const float *hidden = mid + uint64_t(uint(pid)) * uint64_t(args.n_ff) + uint64_t(loop_k);\n"
            "        for (short i = 0; i < 8; i++) {\n"
            ...
            "            const short ly = short((tid / NL1) % 8);\n"
            "            uint kk = uint(8 * sx + i);\n"
            "            *(sb + NK * (8 * sy + ly) + 8 * sx + lx) = (uint(lr1) < uint(nr1) && loop_k + kk < args.n_ff) ? hidden[kk] : half(0.0f);\n"
            "        }\n"
            "        threadgroup_barrier(mem_flags::mem_threadgroup);\n"
            "        auto sA = tA.slice(0, 0);\n"
            "        auto sB = tB.slice(0, 0);\n"
            ...
            "                const short sx = short(2 * il0 + i / 8);\n"
            "                const short lx = short(i % 8);\n"
            "                *(sa + NK * (8 * sy_a + ly_a) + 8 * sx + lx) = k < args.n_ff ? half(dscale * float(scale) * float(q)) : half(0.0f);\n"
            "            }\n"
            "        } else {\n"
            "            for (short i = 0; i < 16; i++) {\n"
            "                const short sx = short(2 * il0 + i / 8);\n"
            "                const short lx = short(i % 8);\n"
            "                *(sa + NK * (8 * sy_a + ly_a) + 8 * sx + lx) = half(0.0f);\n"
            "            }\n"
            "        }\n"
            "        int pid = pair_ids[map_base + uint(lr1)];\n"
            "        uint token = uint(pid) / args.n_active;\n"
