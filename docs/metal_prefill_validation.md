...
- Experimental SSD streaming batch prefill is enabled with
  ...
  is 128 and it is clamped by the expert-cache slot count. Prompts below 64
  tokens keep the token path unless `--streaming-prefill-batch-min N` lowers the
  threshold. The runtime logs the requested, minimum, and effective prefill batch.
